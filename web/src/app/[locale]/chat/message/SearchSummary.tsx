...
export function ShowHideDocsButton({
  ...
  const t = useTranslations("chat_message_SearchSummary");
  return (
    <div
    ...
    >
      {isCurrentlyShowingRetrieved ? (
        <EmphasizedClickable>
          <div className="w-24 text-xs">{t("Hide_Docs")}</div>
        </EmphasizedClickable>
      ) : (
        <BasicClickable>
          <div className="w-24 text-xs">{t("Show_Docs")}</div>
        </BasicClickable>
      )}
    </div>
  );
  ...
  const searchingForDisplay = (
    <div className={`flex p-1 rounded ${isOverflowed && "cursor-default"}`}>
      ...
      <div className="line-clamp-1 break-all px-0.5" ref={searchingForRef}>
        {t("Searching_For")}: <i>{finalQuery}</i>
      </div>
    </div>
  );
